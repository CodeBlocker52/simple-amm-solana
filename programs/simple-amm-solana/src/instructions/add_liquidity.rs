...
pub struct AddLiquidity<'info> {
    ...
    #[account(
        mut,
        seeds = [POOL_SEED, pool.mint_x.as_ref(), pool.mint_y.as_ref()],
        bump = pool.bump,
    )]
...
impl<'info> AddLiquidity<'info> {
    ...
    pub fn handler(&mut self , max_amount_x:u64 , max_amount_y:u64) -> Result<()> {
               ...
               require_gt(user_balance_x , max_amount_x , AmmErrorCode::InsufficientBalance);
               require_gt(user_balance_y , max_amount_y , AmmErrorCode::InsufficientBalance);

               let (amount_x, amount_y)=self.deposit(max_amount_x , max_amount_y)?;
    ...
    pub fn deposit(&mut self , max_amount_x:u64 , max_amount_y:u64) -> Result<(u64, u64)> {
         let pool_balance_x = self.pool_x_ata.amount;
        let pool_balance_y = self.pool_y_ata.amount;
        let lp_supply = self.mint_lp.supply;

        let (amount_x, amount_y)= match lp_supply.eq(&0) && pool_balance_x.eq(&0) && pool_balance_y.eq(&0) {
